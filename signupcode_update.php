...
if (isset($_POST['register_btn'])) {
    ...
    if ($role_as == 'student') {
        $email_check_query = "SELECT student_id_no FROM user WHERE email = ?";
    } elseif ($role_as == 'faculty' || $role_as == 'staff') {
        $email_check_query = "SELECT username FROM faculty WHERE email = ?"; // Assuming username is used for faculty
    }
    ...
    if (mysqli_num_rows($result_verify) > 0) {
        ...
        if ($used == 1) {
            ...
            $update_query = "";
            if ($role_as == 'student') {
                $update_query = "UPDATE user SET lastname = ?, firstname = ?, middlename = ?, gender = ?, course = ?, address = ?, cell_no = ?, birthdate = ?, year_level = ?, role_as = ?, status = 'pending', user_added = NOW(), profile_image = ?, contact_person = ?, person_cell_no = ? WHERE email = ?";
            } elseif ($role_as == 'faculty' || $role_as == 'staff') {
                $update_query = "UPDATE faculty SET lastname = ?, firstname = ?, middlename = ?, gender = ?, course = ?, address = ?, cell_no = ?, birthdate = ?, role_as = ?, status = 'pending', faculty_added = NOW(), profile_image = ?, contact_person = ?, person_cell_no = ? WHERE email = ?";
            }
